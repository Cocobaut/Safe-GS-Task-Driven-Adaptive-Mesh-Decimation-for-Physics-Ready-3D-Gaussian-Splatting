...
def save_pure_ply(xyz, num_point):
        # os.makedirs('view')
        ...
        attributes = xyz # np.concatenate((xyz, normals, offset, color, opacities, scale, rotation), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        
        path = 'kernel_point'
        os.makedirs(path, exist_ok=True)
        path_ply = os.path.join(path, f'kernel_point_{num_point}.ply')
        # path_npz = os.path.join(path, 'head.npz')
...
if __name__=="__main__":
    ...
    num_list = [5, 10, 16, 30, 50]
    for num_point in range(5, 51):
        # out = spherical_Lloyd(radius=1.0, num_cells=num_point, dimension=3, fixed='center', approximation='monte-carlo',
        #                 approx_n=5000, max_iter=500, momentum=0.9, verbose=0)

        out, _ = kernel_point_optimization_debug(radius=1.0, num_points=num_point, num_kernels=1, dimension=3,
                                    fixed='center', ratio=0.66, verbose=0)
    
        save_pure_ply(out[0], num_point)
