...
def scatter_max(src, index, dim=0, out=None, dim_size=None):
    ...
    shape[dim] = dim_size
    out_tensor = torch.full(shape, -float('inf'), dtype=src.dtype, device=src.device) if out is None else out
    out_tensor.scatter_reduce_(dim, index.unsqueeze(-1).expand_as(src) if src.dim() > index.dim() else index, src, reduce="amax", include_self=False)
