...
#pragma unroll
        ...
        if constexpr (GEOMETRY) {
            ...
            const float small_val       = static_cast<float>(normal_len < NORMALIZE_EPS);
            const float large_val       = 1.0f - small_val;
            const float denom           = small_val * NORMALIZE_EPS + large_val * normal_len;
            const glm::vec3 proj        = glm::dot(dL_dpixel_normaln, normaln) * normaln * large_val;
